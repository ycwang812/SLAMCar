...
function ZERONEUIFaceCtrl:Move(moveType, speed)
    ...
    if 0==ZERONE_MotoType then
        if moveType == MoveType.MT_NONE then
            rc_Robot:SetSpeedLeft(0.0)
            rc_Robot:SetSpeedRight(0.0)
        elseif moveType == MoveType.MT_FORWARD then
            rc_Robot:SetSpeedLeft(runSpeed)
            rc_Robot:SetSpeedRight(runSpeed)
        elseif moveType == MoveType.MT_BACKWARD then
            rc_Robot:SetSpeedLeft(-runSpeed)
            rc_Robot:SetSpeedRight(-runSpeed)
        elseif moveType == MoveType.MT_LEFT then
            rc_Robot:SetSpeedLeft(-turnSpeed)
            rc_Robot:SetSpeedRight(turnSpeed)
...
function ZERO_SetLogicPos(obj, logicPos)
    ...
    local mapWidth = rc_Robot:GetMapWidth()
    local mapHeight = rc_Robot:GetMapHeight()
    local mapReso = rc_Robot:GetResolution()

    zo_MapPickPos.x = (percX-0.5) * mapWidth * mapReso
    zo_MapPickPos.y = (percY-0.5) * mapHeight * mapReso
end
...
function ZERONEUIFaceCtrl:CreateBtnsSetting()
    ...
    comBoxMotoType.LocalTransform:SetTranslateY(-3.0)
    ...
    comBoxChara.LocalTransform:SetTranslateY(-3.0)
    ...
    self._btnClearPath = btnClearPath
    self._background:AttachChild(btnClearPath)
    btnClearPath:CreateAddText(""..PX2_LM_APP:GetValue("ClearPath"))
    btnClearPath.LocalTransform:SetTranslateY(-5.0)
    btnClearPath:SetAnchorHor(1.0, 1.0)
    btnClearPath:SetAnchorVer(1.0, 1.0)
    btnClearPath:SetAnchorParamHor(-40.0, -40.0)
    btnClearPath:SetAnchorParamVer(-140.0, -140.0)
    btnClearPath:SetSize(140.0, 30.0)
    btnClearPath:SetPivot(1.0, 0.5)
    btnClearPath:SetScriptHandler("zo_UICallabck")
    ...
    checkShowFace:SetAnchorParamHor(-80.0, -80.0)
    ...
    checkLidar:SetAnchorParamHor(-80.0, -80.0)
    checkLidar:SetAnchorParamVer(100.0, 100.0)
    ...
    checkLidarCur:SetAnchorParamHor(-80.0, -80.0)
    checkLidarCur:SetAnchorParamVer(140.0, 140.0)
    ...
    checkUICtrl:SetAnchorParamHor(-80.0, -80.0)
    checkUICtrl:SetAnchorParamVer(180.0, 180.0)
...
function zo_UICallabck(ptr, callType)
    ...
    elseif UICT_COMBOBOX_CHOOSED==callType then
        if "BtnComboxBoxEdit"==name then    
            local select = ZERONE_UIFaceCtrlScript._btnMapEdit:GetChoose()
            if 0==select then
