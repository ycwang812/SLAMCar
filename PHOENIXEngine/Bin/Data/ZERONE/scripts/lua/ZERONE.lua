...
ZERONE_RobotType = 0 --0 wood 1 meta
...
function zo_ZERONE() 
    ...
    if nil==RobotExt then
        PX2_LOGGER:LogInfo("ZERONE", "Robot:New") 
        rc_Robot = Robot:New()
    else
        PX2_LOGGER:LogInfo("ZERONE", "RobotExt:New")         
    ...
    if 0 == ZERONE_RobotType then
        ...
        rc_Robot:SetMaxSpeed(maxSpeed)
        rc_Robot:SetMaxAccelerate(1.0)
        rc_Robot:SetPredictTime(2.0)
        rc_Robot:SetRadius(0.142)
        rc_Robot:SetLidarIngoreRadius(0.25)
        rc_Robot:SetLidarOffset(-0.12)
        rc_Robot:SetMotoRate(35)
        rc_Robot:SetWheelRadius(0.03)
    elseif 1 == ZERONE_RobotType then
        rc_Robot:SetMinSpeed(0.0)
        rc_Robot:SetMaxSpeed(0.4)
        rc_Robot:SetMaxAccelerate(0.8)
        rc_Robot:SetPredictTime(0.8)
    ...
    if PX2_APP:IsARM() then
        rc_Robot:SetNeedUpdateTexture(false)
    end
    ...
    framePad.LocalTransform:SetTranslateY(-25.0)
