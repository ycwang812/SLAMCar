...
void C3iroboticsLidar::ThreadCall()
{
	bool ret = m_receiver.receivePacket(&m_packet);
	if (ret)
	{
		analysisPacket(m_packet);
	}
}
...
CLidarDynamicScan C3iroboticsLidar::getLidarDynamicScan(void) 
{
	return m_dynamic_scan; 
}
...
TLidarError C3iroboticsLidar::setLidarWorkMode(TLidarWorkMode mode)
{
	...
	if(ret>0)
	{
		starttime = PX2::Time::GetTimeInMicroseconds();

		while(1)
		{		
			PX2::System::SleepSeconds(0.05f);
			...
			endtime = PX2::Time::GetTimeInMicroseconds();
