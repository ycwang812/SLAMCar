...
Others:       None
***********************************************************************************/
TToothScan2 CLidarUnpacket::unpacketLidarScan(CLidarPacket2 &packet)
{
    ...
    tooth_angle = CLidarPacket2::bufToUByte2(buffer);
...
//    printf("[CLidarUnpacket] angle %5.2f, distance_number %d !\n",tooth_scan.angle, distance_number);
    ...
    {
        distance = CLidarPacket2::bufToUByte2(buffer + 2 * i + 2);
...
Others:       None
***********************************************************************************/
TToothScan2 CLidarUnpacket::unpacketLidarScan2(CLidarPacket2 &packet)
{
    TToothScan2 tooth_scan;
    u16 length = packet.getParamLength();
    u8 *buffer = packet.getParamPtr();
...
//    packet.printHex();
    for(size_t i = 0 ; i < distance_number; i++)
    {
        signal = CLidarPacket2::bufToUByte((buffer+2) + 3*i);
        distance = CLidarPacket2::bufToUByte2((buffer+2) + 3*i + 1);
        tooth_scan.distance[i] = float(distance) / 4.0f / 1000.0f;
...
Others:       None
***********************************************************************************/
TToothScan2 CLidarUnpacket::unpacketNewLidarScanNoSingal(CLidarPacket2 &packet)
{
    TToothScan2 tooth_scan;
    ...
    // Get tooth angle, unit is 0.01 degree
    lidar_speed = CLidarPacket2::bufToUByte(buffer + head_ptr_offset);                   head_ptr_offset += 1;
    lidar_offset_angle = CLidarPacket2::bufToByte2(buffer + head_ptr_offset);            head_ptr_offset += 2;
    tooth_angle = CLidarPacket2::bufToUByte2(buffer + head_ptr_offset);                  head_ptr_offset += 2;
...
//    printf("[CLidarUnpacket] angle %5.2f, distance_number %d !\n",tooth_scan.angle, distance_number);
    ...
    for(size_t i = 0 ; i < distance_number; i++)
    {
        distance = CLidarPacket2::bufToUByte2(buffer + 2 * i + head_ptr_offset);
...
Others:       None
***********************************************************************************/
TToothScan2 CLidarUnpacket::unpacketNewLidarScanHasSingal(CLidarPacket2 &packet)
{
    TToothScan2 tooth_scan;
    int head_ptr_offset = 0;
    u16 length = packet.getParamLength();
...
//    packet.printHex();
    for(size_t i = 0 ; i < distance_number; i++)
    {
        signal = CLidarPacket2::bufToUByte((buffer+ head_ptr_offset) + 3*i);
        distance = CLidarPacket2::bufToUByte2((buffer+ head_ptr_offset) + 3*i + 1);
        tooth_scan.distance[i] = float(distance) / 4.0f / 1000.0f;
        tooth_scan.signal[i] = int(signal);
//        printf("[CLidarUnpacket]sigal %d! distance %5.2f\n", tooth_scan.signal[i], tooth_scan.distance[i]);
    }
...
Others:       None
***********************************************************************************/
TLidarError2 CLidarUnpacket::unpacketHealthInfo(CLidarPacket2 &packet)
{
    u8 health_info = *(packet.getParamPtr());
    return TLidarError2(health_info);
}
...
Others:       None
***********************************************************************************/
int CLidarUnpacket::unpacketLidarSpeed(CLidarPacket2 &packet)
{
