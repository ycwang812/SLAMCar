...
namespace everest
{
	namespace hwdrivers
	{
		...
		class CLidarPacketReceiver2
		{
            public:
                /* Constructor */
				CLidarPacketReceiver2();

                /* Destructor */
                ~CLidarPacketReceiver2();

                /* Receive lidar packet, if return true, it means receive a valid packet */
                bool receivePacket(CLidarPacket2 *packet);
			...
			private:
				...
				TPacketResult readPacket(CLidarPacket2 *packet, u8 ch);

				/* process state header1 */
				TPacketResult processStateHeader1(CLidarPacket2 *packet, u8 ch);

				/* process state header2 */
				TPacketResult processStateHeader2(CLidarPacket2 *packet, u8 ch);

				/* Process state length */
				TPacketResult processStateLength(CLidarPacket2 *packet, u8 ch);

				/* Process acuquire data */
				TPacketResult processStateAcquireData(CLidarPacket2 *packet, u8 ch);
