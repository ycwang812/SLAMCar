...
void PathPlan::Optmize()
{
	...
	Vector3f lastDir;
	Vector3f curDir;
	PathingNode *nodeCur0 = 0;
	auto it0 = m_path.begin();
	for (; it0 != m_path.end(); it0++)
	{
		PathingNode *nodeIt = *it0;
		if (!nodeCur0)
		{
		...
		const Vector3f &curPos = nodeCur0->GetPos();

		auto itNext = std::next(it0);
	...
	if (!temp.empty())
	{
		...
		while (it != temp.end())
		{
			...
			{
				// start
				nodeLastUse = nodeCur1;
				temp1.push_back(nodeLastUse);
				it++;
			}
			else
			{
				if (IsCanGo(nodeLastUse, nodeCur1))
				{
					// do nothing
				}
				else
...
//----------------------------------------------------------------------------
void PathingGraph::InitSlamMap(int mapWidth, int mapHeight, float resolution)
{
	DestroyGraph();
	...
	float allWidth = mMapWidth * resolution;
	float allHeight = mMapHeight * resolution;
	...
	for (int y = 0; y < mMapHeight; y++)
	{
		...
		{
			float posX = x * resolution - allWidth*0.5f;
			float posY = y * resolution - allHeight*0.5f;
			...
			int tempNodeAbove = index - mMapWidth;  // reusing tempNode
			if (tempNodeAbove >= 0)
				LinkNodes(m_nodes[tempNodeAbove], pNode, PATHING_DEFAULT_ARC_WEIGHT);

			// link left above
			int tempLeftAbove = tempNodeLeft - mMapWidth;  // reusing left above
			if (tempLeftAbove >= 0)
				LinkNodes(m_nodes[tempLeftAbove], pNode, adjustARC);

			int tempRightAbove = index + 1 - mMapWidth;  // reusing right above
			if (tempRightAbove >= 0)
...
void PathingGraph::SetMapData(const std::vector<unsigned char> mapData,
	int mapDataWidth, int mapDataHeight, float mapDataResolution)
{
	mMapData = mapData;
	mMapDataWidth = mapDataWidth;
	mMapDataHeight = mapDataHeight;
...
bool PathingGraph::IsCanGo(const Vector3f &from1, const Vector3f &to1)
{
	...
	float height = mMapDataHeight * mMapDataResolution;
	...
	float yPerc = from.Y() / height;
	xPerc += 0.5f;
	yPerc += 0.5f;
	float xPercTo = to.X() / width;
	float yPercTo = to.Y() / height;
	xPercTo += 0.5f;
	yPercTo += 0.5f;
...
//----------------------------------------------------------------------------
int PathingGraph::GetMapWidth() const
{
	return mMapWidth;
}
//----------------------------------------------------------------------------
int PathingGraph::GetMapHeight() const
{
	return mMapHeight;
}
//----------------------------------------------------------------------------
float PathingGraph::GetResolution() const
{
	return mResolution;
}
//----------------------------------------------------------------------------
