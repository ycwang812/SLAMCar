...
void ResourceManager::DoResourceUpdateStuffs(const std::string &wwwAddr, 
	const std::string &toPath, const std::string &projName, int type)
{
	...
	std::string filelistPrepare = toPath + projName + "/filelist_prepare.xml";
	std::string filelistToUpdate = toPath + projName + "/filelist_temp.xml";
	std::string filelist = toPath + projName + "/filelist.xml";
	...
	else
	{
		...
		for (; itUpdate != tableToUpdate.end(); itUpdate++)
		{
			...
			if (isNeedDownload)
			{
				...
				std::string downloadPath = toPath + dstDownloadFilename;
