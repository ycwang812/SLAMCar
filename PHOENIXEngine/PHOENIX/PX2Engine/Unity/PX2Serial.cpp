...
void Serial::UpdatePortList()
{
	...
	for (int i = 0; i < (int)ports.size(); i++)
	{
		...
		std::string utf8Desc = StringHelp::UnicodeToUTF8(unicodeDesc);

		wchar_t *unicodeHardID = StringHelp::AnsiToUnicode(hardIDStr.c_str());
		std::string utf8HardID = StringHelp::UnicodeToUTF8(unicodeHardID);

		list.push_back(ports[i].port);
		descList.push_back(utf8Desc);
		hardIDs.push_back(utf8HardID);
#else
		list.push_back(ports[i].port);
