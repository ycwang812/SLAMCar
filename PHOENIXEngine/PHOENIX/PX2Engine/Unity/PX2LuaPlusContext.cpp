...
mLuaPlusState(0)
{
	...
	mEngineUpdate = new LuaPlus::LuaObject();
	mEngineProjectUpdate = new LuaPlus::LuaObject();
...
LuaPlusContext::~LuaPlusContext()
{
	if (mEngineUpdate)
	{
		delete(mEngineUpdate);
		mEngineUpdate = 0;
	}

	if (mEngineProjectUpdate)
	{
		delete(mEngineProjectUpdate);
		mEngineProjectUpdate = 0;
	}
...
//----------------------------------------------------------------------------
bool LuaPlusContext::CallFunction(const std::string &funName, double data0,
	double data1)
{
	...
	if (!funcObj.IsNil())
	{
		...
		{
			LuaObject objdata0;
			objdata0.AssignNumber(mLuaPlusState, data0);

			LuaObject objData1;
			objData1.AssignNumber(mLuaPlusState, data1);
...
//----------------------------------------------------------------------------
bool LuaPlusContext::CallEngineUpdate(double appSeconds, double elapsedSeconds)
{
	if (mEngineUpdate->IsNil())
	{
		*mEngineUpdate = mLuaPlusState->GetGlobal("engine_update");
	}

	if (!mEngineUpdate->IsNil())
	{
		LuaCall call = *mEngineUpdate;

		try
		{
			LuaObject objdata0;
			objdata0.AssignNumber(mLuaPlusState, appSeconds);

			LuaObject objData1;
			objData1.AssignNumber(mLuaPlusState, elapsedSeconds);
...
//----------------------------------------------------------------------------
bool LuaPlusContext::CallProjectUpdate(double appSeconds, double elapsedSeconds,
	bool reGet)
{
	if (reGet)
	{
		*mEngineProjectUpdate = mLuaPlusState->GetGlobal("engine_project_update");
	}

	if (!mEngineProjectUpdate->IsNil())
	{
		LuaCall call = *mEngineProjectUpdate;
	...
	{
		assertion(false, "mEngineProjectUpdate is nil");
	}
