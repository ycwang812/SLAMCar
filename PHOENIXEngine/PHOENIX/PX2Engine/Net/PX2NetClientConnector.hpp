...
namespace PX2
{
	...
	public:
		...
		bool IsHasRawRecvCallback(ClientConnectorRecvCallback callback);
		bool AddRawRecvCallback(ClientConnectorRecvCallback callback);
		bool RemoveRawRecvCallback(ClientConnectorRecvCallback callback);
		void ClearRawRecvCallback();
	...
	protected:
		...
		std::vector<ClientConnectorRecvCallback> mRawCallbacks;
		std::vector<std::string> mRawRecvCallbackStrs;
	...
	private:
		...
		int _ClientOnRead();
		int _ClientOnWrite();
		void _OnRecvRawCallbacks(const std::string &recvStr);
	};
