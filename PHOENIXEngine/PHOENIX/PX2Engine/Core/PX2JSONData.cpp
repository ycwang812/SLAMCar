...
bool JSONData::LoadBuffer(const char *buffer, int size)
{
	...
	if (doc->Parse<rapidjson::kParseValidateEncodingFlag>((char*)buffer, size).HasParseError())
	{
...
#if defined PX2_USE_RAPID_JSON
	rapidjson::Document *doc = (rapidjson::Document *)mDocument;
	int stringLength = doc->GetStringLength();
	return std::string(doc->GetString(), stringLength);
...
bool JSONData::ToStringVal(std::string &value)
{
	...
	if (doc->IsBool())
	{
		value = std::string(doc->GetString(), doc->GetStringLength());
