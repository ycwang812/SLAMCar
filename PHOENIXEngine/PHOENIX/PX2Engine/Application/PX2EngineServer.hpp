...
namespace PX2
{
	...
	public:
		...
		void SendRobotSelfDraw(int clientid);
		void BroadCastSelfDraw();

		void SendLidarData(int clientid);
		void BroadCastLidarData();

		void SendRobotState(int clientid);
	...
	protected:
		...
		int OnRobotToDo(unsigned int clientid, const void *pbuffer, int buflen);

	protected:
		std::map<unsigned int, _ConnectObjPtr> mConnections;
		float mBroadCastMapTiming;
		float mBroadCastSelfDrawTiming;
		float mBroadCastLiDarTiming;
		float mBroadCastRobotStateTiming;
	};
