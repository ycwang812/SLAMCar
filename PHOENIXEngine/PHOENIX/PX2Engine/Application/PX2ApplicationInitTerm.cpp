		...
		else if ("net" == cmd)
		{
			...
			if ("connect" == paramStr1)
			{
				...
				if (cnt)
				{
					...
					cnt->AddRawRecvCallback(_AppClientConnectorRecvCallback);
				}
			}
...
//#endif
	...
	LoadBoost("Data/boostconfig.xml");

	PX2_SC_LUA->CallFile("Data/boostconfig.lua");
...
void Application::_RefreshLocalAddress()
{
	...
	if (addressList.size() > 0)
	{
		for (int i = 0; i < (int)addressList.size(); i++)
		{
			...
			PX2_LOG_INFO("IP:%s", ipAddress.ToString().c_str());
		}
	}
}
//----------------------------------------------------------------------------
...
bool Application::LoadBoost(const std::string &filename)
{
	...
	if (data.LoadBuffer(buffer, bufferSize))
	{
		...
		// local projects
		std::string strBuf;
		PX2_RM.LoadBuffer("Data/projecttopublish.list", strBuf);
		PX2_RM.ClearRes("Data/projecttopublish.list");
		if (!strBuf.empty())
		{
		...
		{
			std::string projectList = writeDataPath + "projecttopublish.list";
			char *buf = 0;
			int bufSize = 0;
		...
		// local plugins
		PX2_RM.LoadBuffer("Data/plugintopublish.list", strBuf);
		PX2_RM.ClearRes("Data/plugintopublish.list");
		if (!strBuf.empty())
		{
		...
		PX2_RM.LoadBuffer("Data/boostproject.list", strBufBoost);
		PX2_RM.ClearRes("Data/boostproject.list");
...
bool Application::WriteBoost()
{
	...
	data.SaveFile("Data/boostconfig.xml");

	_WriteInfos("Data/projecttopublish.list", mBoostInfo.Projects);
	_WriteInfos("Data/plugintopublish.list", mBoostInfo.Plugins);
...
void Application::Update(float appSeconds, float elapsedSeconds)
{
	...
	PX2_SC_LUA->CallEngineUpdate(appSeconds, elapsedSeconds);
	...
	{
		PX2_SC_LUA->CallProjectUpdate(appSeconds, elapsedSeconds, false);
