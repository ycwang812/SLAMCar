...
//----------------------------------------------------------------------------
bool Application::DownloadUpdateProject(const std::string &name, 
	bool doUpdate)
{
	if (name.empty())
		return false;
	...
	if (mBoostInfo.IsDataReWriteToDataUpdate)
	{
		if (!PX2_RM.IsFileFloderExist(projWriteDataPath))
		{
			...
			_WriteInfos(writeDataPath + "projecttopublish.list", mBoostInfoUpdate.Projects);
			_WriteInfos(writeDataPath + "plugintopublish.list", mBoostInfoUpdate.Plugins);
		}
	}
	...
	if (doUpdate)
	{
		...
		std::string writeablePath = PX2_RM.GetWriteablePath();
		std::string toPath = writeablePath + "DataUpdate/";
		PX2_RM.DoResourceUpdateStuffs(addrPath, toPath, name, typeFTPHTTP);
	}
...
bool Application::LoadProject(const std::string &name, bool doUpdate)
{
	...
	DownloadUpdateProject(name, doUpdate);
...
void Application::CloseProject()
{
	...
	PX2_LOG_INFO("Begin Unload Plugins");
...
#endif
	PX2_LOG_INFO("End Unload Plugins");
...
bool Application::LoadPlugins(const std::vector<std::string> &plugins)
{
	for (int i = 0; i < (int)plugins.size(); i++)
	{
		...
		if (!pluginName.empty())
		{
			...
			PX2_LOG_INFO("dll path is %s", path.c_str());
...
{
	PX2_LOG_INFO("Begin ClosePlugins");
