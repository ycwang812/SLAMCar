	...
	else if (UICT_RELEASED == type)
	{
		if (but)
		{
			...
			else if ("ButProjectOpen" == name)
			{
				...
				{
					bool isValied = false;
					std::string strIsCloud = item->GetUserDataString("IsCloud", &isValied);
					if (!isValied)
						strIsCloud = "";
					...
					{
						isDoUpdate = true;
						engineCanvas->SetResUpdatePathFromNet();
					}
					...
					{
						PX2_APP.CloseProject();
						PX2_APP.LoadProject(projName, isDoUpdate);
						PX2_APP.Play(Application::PT_PLAY);
					}
				}
			}
			...
			else if ("BtnLogin" == name)
			{
				...
				UIEditBox *editBoxUsername = DynamicCast<UIEditBox>(
					engineCanvas->GetEngineFrame()->GetObjectByName("EditBoxUsername"));
				std::string username = editBoxUsername->GetText();

				UIEditBox *editBoxPassword = DynamicCast<UIEditBox>(
					engineCanvas->GetEngineFrame()->GetObjectByName("EditBoxPassword"));
				std::string password = editBoxPassword->GetRealText();

				if (!username.empty() && !password.empty())
				{
					engineCanvas->Login(username, password);
				}
			}
			else if ("BtnLogout" == name)
			{
			...
			{
				PX2_GR.PlayNormal(frame);
				std::string projName = frame->GetUserDataString("ProjectName");
				engineCanvas->Download(projName);
			}
		}
	}
...
void EngineCanvas::_CreateEngineFrame()
{
	...
	std::string username = PX2_APP.GetConfig("UserName");
	std::string password = PX2_APP.GetConfig("Password");

	float ver = -20.0f;
	UIEditBox *eidtBoxUsername = UIEditBox::New("EditBoxUsername");
	mEngineFrame->AttachChild(eidtBoxUsername);
	mEditBoxUsername = eidtBoxUsername;
	eidtBoxUsername->LocalTransform.SetTranslateY(-1.0f);
	eidtBoxUsername->SetAnchorHor(0.0f, 0.0f);
	eidtBoxUsername->SetAnchorParamHor(20.0f, 20.0f);
	eidtBoxUsername->SetAnchorVer(1.0f, 1.0f);
	eidtBoxUsername->SetAnchorParamVer(ver, ver);
	eidtBoxUsername->SetPivot(0.0f, 1.0f);
	eidtBoxUsername->SetSize(250.0f, 40.0f);
	eidtBoxUsername->SetText(username);
	UIFText *txtUN = UIFText::New();
	eidtBoxUsername->AttachChild(txtUN);
	txtUN->LocalTransform.SetTranslateY(-2.0f);
	txtUN->SetAnchorHor(0.0f, 1.0f);
	txtUN->SetAnchorVer(0.0f, 1.0f);
	txtUN->GetText()->SetText("username");
	txtUN->SetAlpha(0.4f);
	txtUN->GetText()->SetRectUseage(UIText::RU_ALIGNS);
	txtUN->GetText()->SetAligns(TEXTALIGN_LEFT | TEXTALIGN_VCENTER);
	txtUN->SetAnchorParamHor(5.0f, 0.0f);

	mTextUsername = new0 UIFText();
	mEngineFrame->AttachChild(mTextUsername);
	mTextUsername->LocalTransform.SetTranslateY(-1.0f);
	mTextUsername->SetAnchorHor(0.0f, 0.0f);
	mTextUsername->SetAnchorParamHor(120.0f, 20.0f);
	mTextUsername->SetAnchorVer(1.0f, 1.0f);
	mTextUsername->SetAnchorParamVer(ver, ver);
	mTextUsername->SetPivot(0.0f, 1.0f);
	mTextUsername->SetSize(250.0f, 40.0f);
	mTextUsername->Show(false);
	mTextUsername->GetText()->SetRectUseage(UIText::RU_ALIGNS);
	mTextUsername->GetText()->SetAligns(TEXTALIGN_LEFT | TEXTALIGN_VCENTER);

	UIEditBox *editBoxPassword = UIEditBox::New("EditBoxPassword");
	mEngineFrame->AttachChild(editBoxPassword);
	mEidtBoxPassword = editBoxPassword;
	mEidtBoxPassword->SetPassword(true);
	editBoxPassword->LocalTransform.SetTranslateY(-1.0f);
	editBoxPassword->SetAnchorHor(0.0f, 0.0f);
	editBoxPassword->SetAnchorParamHor(290.0f, 240.0f);
	editBoxPassword->SetAnchorVer(1.0f, 1.0f);
	editBoxPassword->SetAnchorParamVer(ver, ver);
	editBoxPassword->SetPivot(0.0f, 1.0f);
	editBoxPassword->SetSize(250.0f, 40.0f);
	editBoxPassword->SetText(password);
	UIFText *txtPWD = UIFText::New();
	editBoxPassword->AttachChild(txtPWD);
	txtPWD->LocalTransform.SetTranslateY(-2.0f);
	txtPWD->SetAnchorHor(0.0f, 1.0f);
	txtPWD->SetAnchorVer(0.0f, 1.0f);
	txtPWD->GetText()->SetText("password");
	txtPWD->SetAlpha(0.4f);
	txtPWD->GetText()->SetRectUseage(UIText::RU_ALIGNS);
	txtPWD->GetText()->SetAligns(TEXTALIGN_LEFT | TEXTALIGN_VCENTER);
	txtPWD->SetAnchorParamHor(5.0f, 0.0f);

	UIButton *btnLoginOK = UIButton::New("BtnLogin");
	mEngineFrame->AttachChild(btnLoginOK);
	mBtnLogin = btnLoginOK;
	btnLoginOK->LocalTransform.SetTranslateY(-1.0f);
	btnLoginOK->SetAnchorHor(0.0f, 0.0f);
	btnLoginOK->SetAnchorParamHor(560, 460.0f);
	btnLoginOK->SetAnchorVer(1.0f, 1.0f);
	btnLoginOK->SetAnchorParamVer(ver, ver);
	btnLoginOK->SetPivot(0.0f, 1.0f);
	btnLoginOK->SetSize(80.0f, 40.0f);
	btnLoginOK->CreateAddText(PX2_LM_ENGINE.V("Login"));
	btnLoginOK->AddUICallback(_EngineUICallback);

	UIButton *btnLogout = UIButton::New("BtnLogout");
	mEngineFrame->AttachChild(btnLogout);
	mBtnLogout = btnLogout;
	btnLogout->LocalTransform.SetTranslateY(-1.0f);
	btnLogout->SetAnchorHor(0.0f, 0.0f);
	btnLogout->SetAnchorParamHor(20.0f, 20.0f);
	btnLogout->SetAnchorVer(1.0f, 1.0f);
	btnLogout->SetAnchorParamVer(ver, ver);
	btnLogout->SetPivot(0.0f, 1.0f);
	btnLogout->SetSize(80.0f, 40.0f);
	btnLogout->CreateAddText(PX2_LM_ENGINE.V("Logout"));
	btnLogout->AddUICallback(_EngineUICallback);
	mBtnLogout->Show(false);
	...
	canvas->SetAnchorParamVer(20.0f, -80.0f);
	...
	{
		Login(mCfgUsername, mCfgPassword);
	}
}
//----------------------------------------------------------------------------
void EngineCanvas::_RefreshProjects()
{
	...
	for (int i = 0; i < (int)mProjectsCloud.size(); i++)
	{
		const std::string &projName = mProjectsCloud[i];
		UIItem *item = mEngineProjectList->AddItem(projName);
		item->SetName(projName);
		item->GetFText()->GetText()->SetFontColor(Float3::YELLOW);
		...
		cloudBtn->GetPicBoxAtState(UIButtonBase::BS_NORMAL)
			->SetTexture("Data/engine/cloud.png");
		cloudBtn->SetSize(18.0f, 18.0f);
		cloudBtn->SetAnchorParamHor(-20.0f, -20.0f);
		cloudBtn->SetAnchorHor(1.0f, 1.0f);
		cloudBtn->SetColorSelfCtrled(true);
		cloudBtn->SetAlphaSelfCtrled(true);

		auto cloudBtnDownload = UIButton::New("BtnDownload");
		item->AttachChild(cloudBtnDownload);
		cloudBtnDownload->LocalTransform.SetTranslateY(-2.0f);
		cloudBtnDownload->SetSize(120.0f, 32.0f);
		cloudBtnDownload->SetAnchorParamHor(-100.0f, -100.0f);
		cloudBtnDownload->SetAnchorHor(1.0f, 1.0f);
		cloudBtnDownload->SetStateColor(UIButtonBase::BS_NORMAL, Float3(0.6f, 0.6f, 0.6f));
		cloudBtnDownload->SetStateColor(UIButtonBase::BS_HOVERED, Float3(0.6f, 0.6f, 0.6f));
		cloudBtnDownload->CreateAddText("Download");;
		cloudBtnDownload->SetColorSelfCtrled(true);
		cloudBtnDownload->SetAlphaSelfCtrled(true);
		cloudBtnDownload->SetUserDataString("ProjectName", projName);
		cloudBtnDownload->SetUserDataPointer("UIItem", item);
		cloudBtnDownload->GetText()->SetText(PX2_LM_ENGINE.V("Download"));
		cloudBtnDownload->SetStateColor(UIButtonBase::BS_NORMAL, Float3::YELLOW*0.8);
		cloudBtnDownload->SetStateColor(UIButtonBase::BS_HOVERED, Float3::YELLOW*0.8);
		cloudBtnDownload->AddUICallback(_EngineUICallback);

		item->SetUserDataString("IsCloud", "true");
	}
}
//----------------------------------------------------------------------------
void EngineCanvas::Login(const std::string &username,
	const std::string &password)
{
	std::string url = mURL + "/users/login?";
	url += "email=" + username;
	url += std::string("&password=") + password;
	url += std::string("&remember=true");

	CurlObjPtr curlObj = new0 CurlObj();
	curlObj->SetGetType(CurlObj::GT_MEMORY);
	curlObj->Initlize();
	int ret = curlObj->Get(url);
	if (1 == ret)
	{
		char *chMem = curlObj->GetGettedMemory();
		int size = curlObj->GetGettedMemorySize();
		std::string retStr = std::string(chMem, size);

		JSONData jsData;
		if (jsData.LoadBuffer(chMem, size))
		{
			std::string str1 = jsData.ToString();

			if (jsData.IsHasMember("code"))
			{
				JSONValue jsValue = jsData.GetMember("code");
				int val = jsValue.ToInt();
				if (0 == val)
				{
					JSONValue jsMsg = jsData.GetMember("msg");
					std::string strMsg = jsMsg.ToString();

					JSONValue jsValueData = jsData.GetMember("data");
					JSONValue jsValueID = jsValueData.GetMember("id");
					std::string dtStr = jsValueID.ToString();
					mUserID = StringHelp::StringToInt(dtStr);
					...
					mEidtBoxPassword->Show(false);
					mEditBoxUsername->Show(false);
					
					mBtnLogin->Show(false);
					mBtnLogout->Show(true);
					mTextUsername->Show(true);
					mTextUsername->GetText()->SetText(username);

					RefreshCloudProjects(mCfgUsername, mCfgPassword);
					_RefreshProjects();
				}
			}
		}
...
//----------------------------------------------------------------------------
void EngineCanvas::Logout()
{
	if (mCfgUsername.empty() || mCfgPassword.empty())
		return;

	std::string url = mURL + "/users/logout";
	CurlObjPtr curlObj = new0 CurlObj();
	curlObj->SetGetType(CurlObj::GT_MEMORY);
	curlObj->Initlize();
	int ret = curlObj->Get(url);
		...
		if (jsData.LoadBuffer(ret))
		{
			if (jsData.IsHasMember("code"))
			{
				...
				if (0 == val) {
					...
					mEditBoxUsername->Show(true);
					mEidtBoxPassword->Show(true);
					mBtnLogin->Show(true);
					mBtnLogout->Show(false);

					mTextUsername->GetText()->SetText("");
					mTextUsername->Show(false);

					mProjectsCloud.clear();
					_RefreshProjects();
				}
			}
		}
...
//----------------------------------------------------------------------------
void EngineCanvas::RefreshCloudProjects(const std::string &username,
	const std::string &password)
{
	mProjectsCloud.clear();

	// GetCloudProjects
	std::string url = mURL + "/px_projects/projectlist?";
	url += "email=" + username;
	url += std::string("&password=") + password;
	CurlObjPtr curlObj = new0 CurlObj();
	curlObj->SetGetType(CurlObj::GT_MEMORY);
	...
	if (1 == ret)
	{
		...
		JSONData jsData1;
		if (jsData1.LoadBuffer(retStr))
		{
			if (jsData1.IsHasMember("code"))
			{
				JSONValue jsValue1 = jsData1.GetMember("code");

				int val = jsValue1.ToInt();
				if (0 == val)
				{
					if (jsData1.IsHasMember("data")) {

						JSONValue dataJson = jsData1.GetMember("data");
						JSONValue projectsJson = dataJson.GetMember("projects");

						if (projectsJson.IsArray()) {

							for (int i = 0; i < projectsJson.GetArraySize(); i++) {
								JSONValue arrayElement = projectsJson.GetArrayElement(i);

								JSONValue titleJson = arrayElement.GetMember("title");
								std::string strTitle = titleJson.ToString();
								JSONValue titleID = arrayElement.GetMember("id");
								int id = titleID.ToInt();
								
								mProjectsCloud.push_back(strTitle);
							}
						}
					}
				}
...
//----------------------------------------------------------------------------
void EngineCanvas::Download(const std::string &name)
{
	std::string url = mURL + std::string(":80/workspace/")
		+ StringHelp::IntToString(mUserID) + "/M_PHOENIXEngine/";

	PX2_RM.SetResourceUpdateAddr(url);

	PX2_APP.DownloadUpdateProject(name, true);
}
//----------------------------------------------------------------------------
void EngineCanvas::SetResUpdatePathFromNet()
{
	std::string url = mURL + std::string(":80/workspace/")
		+ StringHelp::IntToString(mUserID) + "/M_PHOENIXEngine/";
...
//#if defined (__LINUX__)
	//std::cout << "FPS:" << (int)mFPS << std::endl;
//#endif
}
//----------------------------------------------------------------------------
