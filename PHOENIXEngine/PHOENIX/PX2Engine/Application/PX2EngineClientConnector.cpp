...
mBroadcastTiming(0.0f)
{
	...
	RegisterHandler(EngineServerSendMapSelfDrawMsgID,
		(ServerMsgHandleFunc)&EngineClientConnector::OnMapDataSelfDraw);
	RegisterHandler(EngineServerSendMapSelfDrawEndMsgID,
		(ServerMsgHandleFunc)&EngineClientConnector::OnMapDataSelfDrawEnd);

	RegisterHandler(EngineServerSendLidarMsgID,
		(ServerMsgHandleFunc)&EngineClientConnector::OnLidarData);

	RegisterHandler(EngineServerSendRobotState,
		(ServerMsgHandleFunc)&EngineClientConnector::OnRobotState);
}
//----------------------------------------------------------------------------
EngineClientConnector::~EngineClientConnector()
{
...
{
	PX2_LOG_INFO("OnConnectedToServer");
...
{
	PX2_LOG_INFO("OnDisConnectedToServer");
...
//----------------------------------------------------------------------------
void EngineClientConnector::SendRobotToDo(RobotToDoStruct &robotToDo)
{
	SendMsgToServerBuffer(EngineClientSendRobotToDoID, 
		(const char *)&robotToDo, sizeof(robotToDo));
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
int EngineClientConnector::OnMapDataSelfDraw(const void *pbuffer, int buflen)
{
	Robot::RoleType rt = PX2_ROBOT.GetRoleType();
	if (rt == Robot::RT_CONNECTOR)
	{
		int curSize = mGettingMapSelfDraw.size();
		mGettingMapSelfDraw.resize(curSize + buflen);
		memcpy((char*)&(mGettingMapSelfDraw[0]) + curSize, pbuffer, buflen);
	}

	return 0;
}
//----------------------------------------------------------------------------
int EngineClientConnector::OnMapDataSelfDrawEnd(const void *pbuffer, int buflen)
{
	Robot::RoleType rt = PX2_ROBOT.GetRoleType();
	if (rt == Robot::RT_CONNECTOR)
	{
		...
		if (curSize > 0)
		{
			...
			dstBuf.resize(dstSize);
			PX2_RM.ZipUnCompress((unsigned char*)&(dstBuf[0]), &dstSize,
				(const unsigned char*)&(mGettingMapSelfDraw[0]), curSize);

			PX2_ROBOT.GetCurMapData()->SelfDrawMapData2D.clear();
			PX2_ROBOT.GetCurMapData()->SelfDrawMapData2D = dstBuf;

			PX2_ROBOT.SetSelfDraw(dstBuf);

			mGettingMapSelfDraw.clear();
		}
	}
...
//----------------------------------------------------------------------------
int EngineClientConnector::OnRobotState(const void *pbuffer, int buflen)
{
	Robot::RoleType rt = PX2_ROBOT.GetRoleType();
	if (rt == Robot::RT_CONNECTOR)
	{
...
{
	Robot::RoleType rt = PX2_ROBOT.GetRoleType();
	if (rt == Robot::RT_CONNECTOR_CALCULATE || rt == Robot::RT_CONNECTOR)
	{
